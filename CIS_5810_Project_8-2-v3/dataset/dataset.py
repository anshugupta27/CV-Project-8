...
class ego4dDataset(Dataset):
    ...
    def __init__(self, split, anno_dir, img_dir, transform=None, replace_nan=True):
        self.split = split
        ...
        self.image_size = np.array([224, 224]) # Size of input image to the model
        gt_anno_path = os.path.join(anno_dir, f"ego_pose_gt_anno_{self.split}.json")
    ...
    def __getitem__(self, idx):
        ...
        curr_3d_kpts_cam_offset[~curr_db["valid_flag"]] = 0.0 # Fix: None lead to nan
    ...
    def load_all_data(self, gt_anno_path):
        """
        Store each valid hand's annotation per frame separately:
            - joints_3d
            - valid_flag
            - bbox
            - metadata
    ...
    def inv_normalize_3d(self, kpts):
        return kpts * self.joint_std + self.joint_mean
